...
int llopen(LinkLayer connectionParameters)
{
        ...
        while (alarmCount < totalRetransmitions && current_state != STOP_RCV_open)
        {
            ...
            while (current_state != STOP_RCV_open && alarmEnabled == TRUE)
            {
                if (readByteSerialPort(&byte))
                {
                    ...
                    case START_open:
                        if (byte == FLAG)
                            current_state = FLAG_RCV_open;
                        break;

                    case FLAG_RCV_open:
                        if (byte == A_R)
                            current_state = A_RCV_open;
                        else if (byte == FLAG)
                            break;
                        else
                    ...
                    case C_RCV_open:
                        if (byte == FLAG)
                            current_state = FLAG_RCV_open;
                        else if (byte == (A_R ^ C_UA))
                            current_state = BCC1_OK_open;
                        else
                            current_state = START_open;
                        break;
    ...
    if (current_state != STOP_RCV_open)
        return -1;
    return 1;
}
        ...
        while (current_state != STOP_RCV && alarmEnabled == TRUE)
        {
            if (readByteSerialPort(&byte))
            {
                ...
                case C_RCV:
                    if (byte == FLAG)
                        current_state = FLAG_RCV;
                    else if (byte == (A_R ^ cField))
                        current_state = BCC1_OK;
                    else
                        current_state = START;
                    break;
